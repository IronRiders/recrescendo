...
public class LauncherCommands {
  ...
  public Command launch() {
    return Commands.runOnce(() -> launcher.setTargetVelocity(Constants.Launcher.State.LAUNCH.speed))
        .andThen(Commands.waitUntil(() -> launcher.atSpeed()))
        .andThen(
            Commands.waitSeconds(Constants.Launcher.LAUNCH_TIMEOUT)
                .andThen(() -> launcher.setTargetVelocity(Constants.Launcher.State.STOP.speed)));
  }

  public Command launchGivenSpeed(double target) {
    return Commands.runOnce(() -> launcher.setTargetVelocity(target))
        .andThen(Commands.waitUntil(() -> launcher.atSpeed()))
        .andThen(
            Commands.waitSeconds(Constants.Launcher.LAUNCH_TIMEOUT)
                .andThen(() -> launcher.setTargetVelocity(Constants.Launcher.State.STOP.speed)));
